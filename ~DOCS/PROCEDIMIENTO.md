...
- **Scopar reglas globales sin `:where()`**: reescribir `img { … }` a `.ryc-page img { … }` sube la especificidad de 0,0,1 a 0,1,1 y pisa reglas con una sola clase (`.cofinanciacion-logos`, `.btn-ryc`, `.novedad-banda img`). Para reglas globales (`img`, `a`, `*`) usar siempre `:where(.ryc-page) img { … }` para preservar la especificidad original. Para reglas que **sí** deben ganar sobre el tema (`body`, `html`, `main`) reescribir directo a `.ryc-page` sin `:where()`.
- **Selectores de elemento dentro del HTML entregable** (`h3`, `span`, `button`…): los filtros de Drupal pueden reescribir o quitar atributos/estilos de estos elementos. Para componentes nuevos preferir **`<div>` con clase BEM prefijada `.ryc-*`** (ver `convocatorias.html` como referencia: cards con `.ryc-card`, `.ryc-card__titulo`, `.ryc-card__estado--proxima`…).
